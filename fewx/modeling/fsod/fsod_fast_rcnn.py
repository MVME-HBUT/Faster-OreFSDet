...
from torch import ShortTensor, nn
...
class FsodFastRCNNOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        test_nms_thresh=0.7, #0.5
        test_topk_per_image=100,support_shot = 9 ,rcnn_reduce_dim=256,rpn_reduce_dim =256,gamma = 0.1
    ):
        """
        ...
        dim_in = input_size
        self.conv_1 = nn.Conv2d(dim_in, int(dim_in/2), 1, padding=0, bias=False)
        #self.conv_2 = nn.Conv2d(int(dim_in/2), int(dim_in/4), 1, padding=0, bias=False)
        nn.init.normal_(self.conv_1.weight, std=0.01)
        #self.channel_attention = ParallelPolarizedSelfAttention()
        self.bbox_pred_all = nn.Linear(2048, 4) #num_bbox_reg_classes * box_dim)
        nn.init.normal_(self.bbox_pred_all.weight, std=0.01)
        nn.init.constant_(self.bbox_pred_all.bias, 0)
        
        if self.patch_relation:
            
            #self.bbox_pred_pr = nn.Linear(64*49, 4) #num_bbox_reg_classes * box_dim)
            self.cls_score_pr = nn.Linear(49*49, 2) #nn.Linear(dim_in, 2)
            ############################
            self.rcnn_adapt_k_layer = nn.Linear(int(dim_in/2), rcnn_reduce_dim)
            self.rcnn_adapt_q_layer = nn.Linear(int(dim_in/2), rcnn_reduce_dim)
            #self.rpn_channel_k_layer = nn.Linear(int(dim_in), 1)
            self.rcnn_unary_layer = nn.Linear(int(dim_in/2), 1)
            
        if self.local_correlation:
            #self.conv_cor = nn.Conv2d(int(dim_in/4), int(dim_in/8), 1, padding=0, bias=False)
            self.bbox_pred_cor = nn.Linear(int(dim_in/2), 4)
            self.cls_score_cor = nn.Linear(int(dim_in/2), 2) #nn.Linear(dim_in, 2)
            #self.cls_score_cor_change = nn.Linear(int(dim_in/2), 64)

        if self.global_relation:
            #self.fc_1 = nn.Linear(int(dim_in) , int(dim_in/32))
            #self.fc_2 = nn.Linear(int(dim_in/32), int(dim_in/128))
            #self.bbox_pred_fc = nn.Linear(int(dim_in), 4)
            self.cls_score_fc = nn.Linear(int(dim_in), 2) #nn.Linear(dim_in, 2)     #(int(dim_in/32)      
        ...
        if self.patch_relation:
            ...
            #self.rcnn_transform_layer = nn.Linear(1024, 64)
            nn.init.normal_(self.rcnn_adapt_k_layer.weight, std=0.01)
            nn.init.constant_(self.rcnn_adapt_k_layer.bias, 0)
            nn.init.normal_(self.rcnn_adapt_q_layer.weight, std=0.01)
            nn.init.constant_(self.rcnn_adapt_q_layer.bias, 0)
        ...
        if self.local_correlation:
            ...
            nn.init.constant_(self.bbox_pred_cor.bias, 0)
            # nn.init.normal_(self.cls_score_cor_change.weight, std=0.001)
            # nn.init.constant_(self.cls_score_cor_change.bias, 0)
        ...
        if self.global_relation:
            ...
            # nn.init.constant_(self.bbox_pred_fc.bias, 0)
        ###########################################################
    ...
    def forward(self, x_query, x_support):
        ...
        #  x_support  [1, 2048, 7, 7] x_query  [128, 2048, 7, 7]
        #box
        #bbox_pred_a =  self.bbox_pred_all(x_query.mean(3).mean(2))  # [128, 2048]
        #cls
        x_support = F.relu(self.conv_1(x_support), inplace=True)# [1, 1024, 7, 7]
        x_query = F.relu(self.conv_1(x_query), inplace=True)# [128, 1024, 7, 7]
        #############################################################
        
        if self.global_relation:
            
            x_support_fc = x_support.expand_as(x_query)
            cat_fc = torch.cat((x_query, x_support_fc), 1)
            cat_fc = self.avgpool_fc(cat_fc).squeeze(3).squeeze(2) #[128, 1024, 1, 1]
            cls_score_fc = self.cls_score_fc(cat_fc)
            #bbox_pred_fc = self.bbox_pred_fc(cat_fc)

        # correlation
        if self.local_correlation:
            
            #channel_weight = self.channel_attention (x_query,x_support_fc)
            #channel_att=(channel_weight*x_query).mean(dim=[2, 3], keepdim=True).squeeze(3).squeeze(2)
            
            x_cor = F.relu(F.conv2d(x_query, x_support.permute(1,0,2,3), groups=1024), inplace=True).squeeze(3).squeeze(2) # [128, 1024] #.squeeze(3).squeeze(2)
            #print(x_cor.shape)
            #print(channel_att.shape)
            #x_cor = channel_att + x_cor
            #x_cor = F.relu(F.conv2d(x_query, x_support.permute(1,0,2,3), groups=1024), inplace=True).squeeze(3).squeeze(2) # [128, 1024]
            bbox_pred_cor = self.bbox_pred_cor(x_cor)
            cls_score_cor = self.cls_score_cor(x_cor)
            
        #patch
        if self.patch_relation:
            # combined_feat = []
            # x_support_gr = x_support.expand_as(x_query)
            # batch_size = x_support_gr.size(0)
            # for _vec, _roi_feat in zip(x_support_gr.chunk(batch_size, dim=0), x_query.chunk(batch_size, dim=0)):
            #     concat_feat = torch.cat([_vec.repeat(_roi_feat.size(0), 1, 1, 1), _roi_feat], 1)
            #     combined_feat += [concat_feat]
            # combined_feat = torch.cat(combined_feat, 0)  # [B*128, 2048, 7, 7]
            # combined_feat = self.avgpool_fc(combined_feat).squeeze(3).squeeze(2) #[128, 1024, 1, 1]
            # cls_score_pr = self.cls_score_pr(combined_feat)
            # bbox_pred_a =  self.bbox_pred_all(combined_feat)  # [128, 2048]
            n_roi = x_query.size(0)    
            support = x_support.view(1, 1024, -1).transpose(1, 2)  # [1, 49, 512]
            query = x_query.view(n_roi, 1024, -1).transpose(1, 2)  # [128, 49, 512]
            support = support.repeat(query.size(0), 1, 1)  # [128, 49, 512]
            support = self.pos_encoding_layer(support)
            query = self.pos_encoding_layer(query)
            k_matrix = self.rcnn_adapt_k_layer(support)  # [128, 49, 64]  k矩阵
            k_matrix = k_matrix - k_matrix.mean(1, keepdim=True)
            q_matrix = self.rcnn_adapt_q_layer(query)  # [128, hw, 64]
            q_matrix = q_matrix - q_matrix.mean(1, keepdim=True)
            support_adaptive_attention_weight = torch.bmm(q_matrix, k_matrix.transpose(1, 2)) / math.sqrt(self.rcnn_reduce_dim)  # [B, 49, 49] 
            #print(support_adaptive_attention_weight.shape) 
            support_adaptive_attention_weight = F.softmax(support_adaptive_attention_weight, dim=2) 
            #print(support_adaptive_attention_weight.shape) 
            cls_score_pr = self.cls_score_pr(support_adaptive_attention_weight.view(n_roi, -1))
            
        ##################################################################################
        # final result
        #print(cls_score_pr.shape)
        #print(cls_score_cor.shape)
        #print(cls_score_fc.shape)
        cls_score_all =    cls_score_cor/0.1 + cls_score_fc + cls_score_pr # #+ cls_score_pr)/0.2 ###########放大系数
        # cls_score_all =  (cls_score_cor + cls_score_fc + cls_score_pr)/0.2 ###########放大系数
        bbox_pred_all =    bbox_pred_cor/0.1 #+ bbox_pred_a # [128, 2048]       ##########放大系数
        
        # bbox_pred_all =  (bbox_pred_cor + bbox_pred_fc)/0.1 #+ bbox_pred_cor + bbox_pred_a # [128, 2048]       ##########放大系数
        #cls_score_all = F.softmax(cls_score_all, 1)
